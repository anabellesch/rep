...
public class TClasificador {
	...
	public static final int METODO_CLASIFICACION_QUICKSORT_CAMINOS = 6;
	...
	public TCamino[] clasificarCaminos(TCamino[] datosParaClasificar, int metodoClasificacion) {
		switch (metodoClasificacion) {
			case METODO_CLASIFICACION_QUICKSORT_CAMINOS:
				return ordenarPorQuickSortCaminos(datosParaClasificar);
			default:
				System.err.println("Este codigo no deberia haberse ejecutado");
				break;
		}
	...
	protected TCamino[] ordenarPorQuickSortCaminos(TCamino[] datosParaClasificar) {
		quicksortCaminos(datosParaClasificar, 0, datosParaClasificar.length - 1);
		return datosParaClasificar;

	}

	private void quicksortCaminos(TCamino[] entrada, int i, int j) {
		int izquierda = i;
		int derecha = j;

		int posicionPivote = encuentraPivoteCaminos(izquierda, derecha, entrada);
		if (posicionPivote >= 0) {
			double pivote = entrada[posicionPivote].getCostoTotal();
			while (izquierda <= derecha) {
				while ((entrada[izquierda].getCostoTotal() < pivote) && (izquierda < j)) {
					izquierda++;
				}

				while ((pivote <= entrada[derecha].getCostoTotal()) && (derecha > i)) {
					derecha--;
				}

				if (izquierda <= derecha) {
					TCamino temp = entrada[izquierda];
					entrada[izquierda] = entrada[derecha];
					entrada[derecha] = temp;
					izquierda++;
					derecha--;
				}
			}
		...
		if (i < derecha)
			quicksortCaminos(entrada, i, derecha);
		if (izquierda < j)
			quicksortCaminos(entrada, izquierda, j);
	}

	private int encuentraPivoteCaminos(int izquierda, int derecha, TCamino[] entrada) {
		int pivote = -1;
		if (entrada[izquierda].getCostoTotal() > entrada[izquierda + 1].getCostoTotal()) {
			pivote = izquierda;
		} else {
			pivote = izquierda + 1;
		}
